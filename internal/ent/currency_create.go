...
func (_c *CurrencyCreate) SetDecimalPlaces(v int) *CurrencyCreate {
	_c.mutation.SetDecimalPlaces(v)
	return _c
}

// SetNillableDecimalPlaces sets the "decimal_places" field if the given value is not nil.
func (_c *CurrencyCreate) SetNillableDecimalPlaces(v *int) *CurrencyCreate {
	if v != nil {
...
func (_c *CurrencyCreate) AddBudgetIDs(ids ...int) *CurrencyCreate {
	_c.mutation.AddBudgetIDs(ids...)
	return _c
}

// AddBudgets adds the "budgets" edges to the Budget entity.
func (_c *CurrencyCreate) AddBudgets(v ...*Budget) *CurrencyCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_c *CurrencyCreate) defaults() {
	if _, ok := _c.mutation.DecimalPlaces(); !ok {
		v := currency.DefaultDecimalPlaces
		_c.mutation.SetDecimalPlaces(v)
	}
}
...
func (_c *CurrencyCreate) check() error {
	...
	if _, ok := _c.mutation.DecimalPlaces(); !ok {
		return &ValidationError{Name: "decimal_places", err: errors.New(`ent: missing required field "Currency.decimal_places"`)}
	}
	return nil
}
