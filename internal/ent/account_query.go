...
func (_q *AccountQuery) QueryCountry() *CountryQuery {
...
func (_q *AccountQuery) QueryCurrency() *CurrencyQuery {
...
func (_q *AccountQuery) WithCountry(opts ...func(*CountryQuery)) *AccountQuery {
	query := (&CountryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) WithCurrency(opts ...func(*CurrencyQuery)) *AccountQuery {
	query := (&CurrencyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if _q.withOwner != nil || _q.withCountry != nil || _q.withCurrency != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withCountry; query != nil {
		if err := _q.loadCountry(ctx, query, nodes, nil,
			func(n *Account, e *Country) { n.Edges.Country = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadCountry(ctx context.Context, query *CountryQuery, nodes []*Account, init func(*Account), assign func(*Account, *Country)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Account)
	for i := range nodes {
...
func (_q *AccountQuery) loadCurrency(ctx context.Context, query *CurrencyQuery, nodes []*Account, init func(*Account), assign func(*Account, *Currency)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Account)
	for i := range nodes {
